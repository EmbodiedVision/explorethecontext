...
def is_context_set_informative(transition_fcn, x, u, x_next, assignment, batchsize):
    # Context set is informative if it contains at least
    # one informative transition
    is_informative = transition_fcn.is_transition_informative(x, u, x_next).astype(
        np.float32
    )
    set_is_informative = torch.zeros(batchsize).float()
    set_is_informative.index_add_(
        dim=0, index=torch.tensor(assignment), source=torch.tensor(is_informative)
    )
    set_is_informative = set_is_informative > 0.01
    return set_is_informative.numpy()
...
def plot_context_prediction(
    ...
    informative_entropy = context_entropy[is_informative]
    noninformative_entropy = context_entropy[~is_informative]
    ax.scatter(context_size[is_informative], informative_entropy)
    ax.scatter(context_size[~is_informative], noninformative_entropy)
    ...
    if writer:
        writer.add_figure("fig/context_set_informative", fig, step, close=False)
    plt.close()
...
def generate_prediction_rollout(validation_rollout, transition_model, context_latent):
    ...
    state = torch.tensor(validation_rollout["observation"][0]).float().to(device)
    state = state.unsqueeze(0)
    actions = (
        torch.stack([torch.Tensor(a) for a in validation_rollout["action"]])
        .float()
        .to(device)
        .unsqueeze(1)
    )
    ...
    observation_distribution = transition_model.forward_multi_step(
        state, actions, context_latent, return_mean_only=True
    )
...
def plot_rollouts(
    ...
    envs = [make_env(env_name) for _ in range(n_rollouts)]
