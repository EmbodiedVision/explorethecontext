...
class QuadraticResize(transforms.Resize):
    def __init__(self, size, interpolation=Image.BILINEAR):
        if isinstance(size, int):
            super(QuadraticResize, self).__init__((size, size), interpolation)
        elif isinstance(size, tuple):
            assert len(size) == 2
            assert size[0] == size[1]
            assert isinstance(size[0], int) and isinstance(size[1], int)
            super(QuadraticResize, self).__init__(size, interpolation)
        else:
            raise ValueError(f"Invalid type {type(size)} of argument 'size'")
