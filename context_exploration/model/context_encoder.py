...
class LinearNonNegativeWeightsSoftplus(LinearNonNegativeWeights):
    def __init__(self, *args, **kwargs):
        super(LinearNonNegativeWeightsSoftplus, self).__init__(*args, **kwargs)

    @property
    def weight(self):
        if LinearNonNegativeWeightsSoftplus.CLAMP:
            return F.softplus(torch.clamp(self._weight, min=-3))
        else:
            return F.softplus(self._weight)
...
class ContextEncoder(nn.Module):
    ...
    def empty_set_context(self):
        ...
        return Normal(context.loc.squeeze(0), context.scale.squeeze(0))
...
class MLPContextEncoder(ContextEncoder):
    def __init__(
        ...
        LinearNonNegativeWeightsSoftplus.CLAMP = clamp_softplus_weights

        if positive_weights == "relu":
            layer_class = LinearNonNegativeWeightsReLU
        elif positive_weights == "softplus":
            layer_class = LinearNonNegativeWeightsSoftplus
        elif positive_weights == True:  # for backwards compatibility
            layer_class = LinearNonNegativeWeightsSoftplus
