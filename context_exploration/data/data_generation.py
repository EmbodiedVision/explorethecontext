...
class ConcatEnv(gym.Env):
    ...
    We deliberately do not implement all abstract methods from gym.Env,
    as this is only to be used in a StorageWrapper, which only needs
...
def generate_rollout(env_name: str, seed: int, experience_dir: Path):
    env_1 = make_env(env_name)
    env_2 = make_env(env_name)
...
def generate_data():
    ...
    for env_name in ENVS.keys():
        ...
        # n_env_samples: N_TRAIN + N_VAL
        if "action_squash" in env_name:
            n_env_samples = 5_000 + 1_000
        elif "mountaincar" in env_name:
            n_env_samples = 50_000 + 10_000
        else:
            n_env_samples = 100_000 + 10_000
        ...
        for seed in range(n_env_samples):
